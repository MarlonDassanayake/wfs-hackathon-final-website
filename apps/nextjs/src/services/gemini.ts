 ...
 *
 * ⚠️  The API key lives in .env.local (NEXT_PUBLIC_GEMINI_API_KEY).
 *     .env.local is gitignored. Never hard-code the key here.
 */
...
async function callGemini(prompt: string): Promise<string> {
  if (!API_KEY) throw new Error('NEXT_PUBLIC_GEMINI_API_KEY is not set in .env.local');
